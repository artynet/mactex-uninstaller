...
#
# WARNING:      This script will remove all files and folders created by MacTex-2012.
#               And if MacTex-2012 and other TeX exist, this script will not work well.
#
...
MACTEX_TEXLIVE='org.tug.mactex.texlive2012'
MACTEX_GHOSTSCRIPT='org.tug.mactex.ghostscript9.05'
MACTEX_IMAGEMAGICK_CONVERT='org.tug.mactex.imagemagick-convert-6.7.6-9'
...
if [ ! -z "`echo ${PKGS} | grep ${MACTEX_TEXLIVE}`" ]; then
    echo "Remove TeXLive 2012..."
    remove_item "${VOLUME}usr/local/texlive/2012"
    remove_dir_if_empty "/usr/local/texlive"
    if [ ${#} -eq 2 ]; then
        echo "WARNING: Removed texlive-2012, but other version of TeX exist."
...
if [ ! -z "`echo ${PKGS} | grep ${MACTEX_IMAGEMAGICK_CONVERT}`" ]; then
    ...
    remove_dir_if_empty "${VOLUME}usr/local/lib/ImageMagick-6.7.6"
